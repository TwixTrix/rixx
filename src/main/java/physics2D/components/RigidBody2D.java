...
public class RigidBody2D extends Component {
    ...
    public void update(float dt)
    {
        if(rawBody != null)
        {
           ...
           {
               this.rawBody.setTransform(new Vec2(this.gameObject.transform.position.x, this.gameObject.transform.position.y) , (float)Math.toRadians(this.gameObject.transform.rotation));
           }
        }
    }
    ...
    {
        this.velocity.set(new Vector2f( x,  y));
        if(rawBody != null)
        {
            this.rawBody.setLinearVelocity(new Vec2(x,y));
        }
